...
## Spyder 使用範例
在 Spyder 內可直接執行 `scripts/spyder_run.py` 並呼叫函式：

```python
from scripts.spyder_run import (
    init_db,
    sync_watchlist_spyder,
    fetch_candidates_spyder,
    run_similarity_spyder,
    write_report_spyder,
)

init_db()
sync_watchlist_spyder(start="2024-01-01", end="2024-12-31", incremental=False)
fetch_candidates_spyder(start="2024-10-01", end="2024-10-31")
run_id = run_similarity_spyder()
write_report_spyder(run_id, fmt="excel")
```
